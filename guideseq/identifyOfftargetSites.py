...
class chromosomePosition():
    ...
    def addPosition(self, chromosome, position, strand):
        # Create the chromosome keyValue if it doesn't exist
        if chromosome not in self.chromosome_dict:
            self.chromosome_dict[chromosome] = {}
        # Increment the position on that chromosome if it exists, otherwise initialize it with 1
        if position not in self.chromosome_dict[chromosome]:
            self.chromosome_dict[chromosome][position] = {}
            self.chromosome_dict[chromosome][position]['+_total'] = 0
            self.chromosome_dict[chromosome][position]['-_total'] = 0
            self.chromosome_dict[chromosome][position]['+'] = collections.Counter()
            self.chromosome_dict[chromosome][position]['-'] = collections.Counter()

        self.chromosome_dict[chromosome][position][strand + '_total'] += 1
    ...
    # Generates a summary by position
    def SummarizePositions(self):
        self.position_summary = [[chromosome, position,
                                          len(self.chromosome_dict[chromosome][position]['+']),
                                          len(self.chromosome_dict[chromosome][position]['-']),
                                          self.chromosome_dict[chromosome][position]['+_total'],
                                          self.chromosome_dict[chromosome][position]['-_total'],
                                          ]
                                         for chromosome in sorted(self.chromosome_dict)
                                         for position in sorted(self.chromosome_dict[chromosome])]
        return self.position_summary

    # Summarizes the chromosome, positions within a 10 bp window
    def SummarizeIndex(self):
        last_chromosome, last_position, window_index = 0, 0, 0
        index_summary = []
        for chromosome, position, plus_count, minus_count, total_plus_count, total_minus_count, in self.position_summary:
        ...
        for index in self.index_stack:
            sorted_list = sorted(self.index_stack[index], key=operator.itemgetter(4))   # sort by barcode_count_total #TODO??
            chromosome_list, position_list, total_plus_count_list, total_minus_count_list, total_sum_list = zip(*sorted_list)
            ...
            BED_name = BED_format_chromosome + "_" + str(most_frequent_position) + "_bc"
            offtarget_sequence = self.getSequence(self.genome, most_frequent_chromosome, most_frequent_position - 35, most_frequent_position + 35)
...
def analyze(sam_filename, reference_genome, outfile, annotations):
    ...
    for line in file:
        ...
        if len(fields) >= 10:
            ...
            if int(mapq) >= 50 and int(sam_flag) & 128 and not int(sam_flag) & 2048: #dups??
                if int(template_length) < 0:  # Reverse read
                    read_position = int(position_of_mate) + abs(int(template_length)) - 1
                    strand = "-"
                    chromosome_position.addPosition(chromosome, read_position, strand)
    ...
    stacked_summary = chromosome_position.SummarizePositions()
    ...
    with open(outfile, 'w') as f:
        ...
        f.write('\t'.join(['#BED Chromosome', 'BED Min.Position',
                           ...
                           '+.total', '-.total', 'total.sum', 'total.geometric_mean',
        ...
        summary = chromosome_position.SummarizeIndex()
        target_sequence = annotations["Sequence"]

        for row in summary:
            #minimum params for output - 1 read in each direction
            if int(row[8]) < 1 or int(row[9]) < 1: # or int(row[10]) < 10:
            # if int(row[10]) < 10:  #skip if less than 10 supporting reads
                 continue
            ...
            if target_sequence:
                ...
                sys.stderr.write("sequence: " + sequence + "\n")
                sys.stderr.write("mismatches: " + str(mismatches) + "\n")
                sys.stderr.write("length: " + str(length) + "\n")
                sys.stderr.write("strand: " + strand + "\n")
                ...
                padding=35
                if strand == "+":
                    target_start_absolute = target_start_relative + int(row[2]) - padding
                    target_end_absolute = target_end_relative + int(row[2]) - padding
                elif strand == "-":
                    target_start_absolute = int(row[2]) + padding - target_end_relative
                    target_end_absolute = int(row[2]) + padding - target_start_relative
                else:
                ...
                f.write('\t'.join([str(x) for x in row[4:8] + [filename_tail] + row[0:4] + row[8:] + [""] * 9] + ['\n'])) #+ annotation] + ['\n']))
